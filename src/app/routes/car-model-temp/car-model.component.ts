...
export class CarModelComponent extends CustomPageOperationComponent implements OnInit {
  ...
  public status = {
    ...
    modelTools: [
      [
        ...
        { icon: 'icon-stereoscopic', key: 'peel', isActive: false, title: '剥离', isAllowActive: false, isStatus: true },
  ...
  private install() {
    ...
    let modelType = this.routeInfo.snapshot.queryParams['menutype'];

    if (modelType == 'chassis') {
  ...
  private modelRender(path?: string) {
    ...
    // modelPath = "http://file.car.typeo.devp/00000000-0000-0000-0000-000000000000/bb4df678d3a1cd27fb21fd724e6a07cf/";
  ...
  private lmenuClick(menu: any) {
    if (this.routeInfo.snapshot.queryParams['menutype'] != 'chassis' && menu.type == 'carbody') {
      ...
      Utils.openPath('/car-model?' + Status.carModelId + '=' + this.status.modelId + '&menutype=' + menu.type);
    }
  }
    ...
    if (toolKey == 'bind' || toolKey == 'unbind') {
      ...
      if (((!treeNode || !treeNode.length) || !element) && toolKey == 'bind') {
        ...
        if (toolKey == 'bind') {
            ...
            if (res.success) {
              ...
              if (parentNodeOrigin) {
                if (isCount) {
                  ...
                  // 更新父级统计数字
                  let parents = tree.parentNode.parentNode;
                  if (parents) {
                    let parentsOrigin = parents.origin;
                    let parentsChilds = parents.children;
                    let allBindCount = 0;
                    let childLength = parentsChilds.length;
                    Utils.forEach(parentsChilds, (childItem) => {
                      allBindCount += childItem.origin['bindCounts'];
                      childLength += childItem.origin.children.length;
                    });
                    parentsOrigin['bindCounts'] = allBindCount;
                    parentsOrigin['title'] = parentsOrigin['name'] + '(' + (allBindCount) + '/' + (childLength) + ')';
                    tree.parentNode.parentNode['title'] = parentsOrigin['title'];
                  }
                }
                // 修改文本
          ...
          // params['modelId'] = ((tree.origin || {}).modelId || [])[0];
      ...
      if (currentTool.isActive) {
        ...
        if (currentTool['method'] != false) {
          if (currentTool['isStatus']) {
            let toolStatus = currentTool['status'];
            selector.setMethod(toolKey, !toolStatus);
            currentTool['status'] = !toolStatus;
          }
          else {
  ...
  private nzTreeCheck(event: NzFormatEmitEvent): void {
    ...
    if (node.isChecked) {
      ...
      (status.bindData.element || []).forEach((item) => {
        itemFlag = false;
        Utils.forEach(modelIdList, (listItem) => {
          if (item == listItem) {
            itemFlag = true;
          }
        })
  ...
  public addAttributeDirectory() {
    ...
    if (!tree && this.type != -1 && this.type != 0) {
      return;
    }
   ...
   */
  private editAttributeDirectory(attributeDirectory) {
    this.operationOpenModal({
      operationModalTitle: '编辑属性目录',
      infoModel: this.status.newPropertyModel,
      customForms: this.customForms,
      formData: {name: attributeDirectory.name}
    }).submit(() => {
      let params = this.customForms.getModelJson();
      params['id'] = attributeDirectory.id;
      params['level'] = attributeDirectory.level;
      params['parentId'] = attributeDirectory.parentId;
      params['projectId'] = attributeDirectory.projectId;
      params['sort'] = attributeDirectory.sort;
      params['type'] = attributeDirectory.type;
      // 类型 -1.整车 0.车身 1.总成 2.零件
      ...
      this.formSubmit({
        ...
        if (res.success) {
          attributeDirectory['name'] = params['name'];
