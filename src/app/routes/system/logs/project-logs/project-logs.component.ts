...
export class ProjectLogsComponent extends CustomPageOperationComponent implements OnInit {
  ...
  public loadData() {
    this.loadDatas({
      ...
      params: {
        ...
        "skipCount": this.status.pageIndex,
      ...
      if (res.success) {
        ...
        //   // console.info(res.result.items[i].browserInfo);
        //   let length = 69;
        //   // res.result.items[i].browserInfo = res.result.items[i].browserInfo.split(/\(.*?\)/).join(",")
        // }
  ...
  public async pageIndexChange(index: any) {
    // console.log(index)
    this.status.pageIndex = index;
    this.loadData();
  }
