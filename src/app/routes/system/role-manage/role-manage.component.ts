...
@Component({
  ...
  styleUrls: ['./role-manage.component.scss', './../../../components/custom-forms/custom-forms.component.scss']
})

export class RoleManageComponent extends CustomPageOperationComponent implements OnInit {
  ...
  public status = {
    ...
    roleInfoModal: [
      // 角色名称
      {
        value: null, text: '角色名称', key: 'displayName', require: true, requireIconType: 'static', maxWidth: true, maxlength: 20
      },
      // 职能描述
      {
        value: null, text: '职能描述', type: 'textarea', key: 'description', rowType: 'static', maxlength: 100
      }
    ],
    // 权限分配
    authority: [
  ...
  private disposePermissionsData(permissions, grantedPermissionNames) {
    // 获取首级
    let FirstPermissions = []; let tempPermission = {};
    ...
    Utils.resetData(permissions, (permission) => {
      ...
      if (authorityObjs[permissionKey]) {
        tempPermission[permissionKey] = tempPermission[permissionKey] || {text: null, key: null, group: []};
        ...
        for (let key in authorityObjs) {
          if (permissionKey.indexOf(key) > -1) {
            ...
            groupItem.group.push({value: permissionKey, label: permission.displayName, key: permissionKey, checked: checkStatus});
          }
        }
      }
    });
    ...
    this.status.authority = FirstPermissions;
  ...
  private getChoosedRoleAuthority() {
    let resultAuthority = [];
    // 获取全部权限
    let grantedPermissions = eval('(' + Utils.getLocalStorage(Status.grantedPermissions) + ')');
    // 除设置的权限外，其他权限全部加入
    ...
    let i = 0, len = authority.length, authorityItem;
    for(;i<len;i++){
      authorityItem = authority[i];
      let group = authorityItem.group;
      let j = 0, lenJ = group.length, groupItem;
      for(;j<lenJ;j++){
        groupItem = group[j];
  ...
  private add() {
    this.operationOpenModal({
      ...
      this.formSubmit({
        customForms: this.customForms,
        params: {
          role: this.customForms.merge({isDefault: true, id: null}, this.customForms.getModelJson()),
  ...
  private edit(data: any) {
    ...
    this.operationOpenModal({
      ...
      this.formSubmit({
        customForms: this.customForms,
        params: {
          role: this.customForms.merge({isDefault: data.isDefault, id: data.id}, this.customForms.getModelJson()),
          grantedPermissionNames: this.getChoosedRoleAuthority()
        },
        // 修改接口方法名称
