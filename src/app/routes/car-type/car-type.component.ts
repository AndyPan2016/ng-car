...
import { Utils, Lang, Status, KeyCodeEvent } from '../../common/helper/util-helper';
...
export class CarTypeComponent extends CustomPageOperationComponent implements OnInit {
  @ViewChild('customForms') public customForms: CustomFormsComponent;
  @ViewChild('customFormsByTypeInfo') public customFormsByTypeInfo: CustomFormsComponent;
  ...
  public status = {
    // 新建、修改车型信息表单数据
    typeInfoModal: [
      {value: null, text: '车型名称', key: 'name', require: true, maxlength: 32},
      {value: null, text: '车型编号', key: 'code', require: true, maxlength: 32},
      {
        value: null, text: '车型级别', type: 'select-groups', key: 'level', require: true,
        rowType: 'column-row-2', placeholder: '车型级别', groups: Status.carLevel
        // [
        //   {value: 1, text: '轿车'},
        //   {value: 2, text: 'SUV'},
        //   {value: 3, text: 'MVP'}
      ...
      {
        value: null, text: '整车结构', type: 'select', key: 'structure', require: true,
        rowType: 'column-row-2 right', placeholder: '整车结构', options: [
          {value: 1, text: '承载式'},
          {value: 2, text: '非承载式'}
        ]
      },
      {
        value: null, text: '燃油类型', type: 'select', key: 'category', require: true,
        rowType: 'column-row-2', placeholder: '燃油类型', options: Status.carCategory
        // [
        //   {value: 0, text: '汽油'},
        //   {value: 1, text: '柴油'},
        //   {value: 2, text: '充电'},
      ...
      {
        value: null, text: 'SOP时间', type: 'date-month', key: 'sopTime',
        rowType: 'column-row-2 right', placeholder: 'SOP时间', require: true
      },
      {value: null, text: '车型厂家', key: 'CompanyName', maxlength: 32},
      {
        value: null, text: '车型照片', type: 'upload-image', key: 'url', rowType: 'static', isCreate: false,
        // require: true,
        // 指定上传文件类型，和上传文件大小限制（单位：兆(M)）
        limitFileType: 'image', limitFileSize: 10
      }
    ],
    // isCreateForm: false,
    // 赛选条件模型
    filterModal: [
      {
        value: null, text: '车型级别', type: 'select-groups', key: 'level', groups: Status.carLevel
      },
      {
        value: null, text: '燃油类型', type: 'select', key: 'category', options: Status.carCategory
        // [
        //   {value: 1, text: '汽油'},
        //   {value: 2, text: '柴油'},
        //   {value: 3, text: '充电'},
      ...
      {
        value: null, text: '车型结构', type: 'select', key: 'structure', options: [
          {value: 1, text: '承载式'},
          {value: 2, text: '非承载式'}
        ]
      }
  ...
  private install() {
    ...
    let tools = this.status.tools; let temp = [];
   ...
   */
  private loadData() {
    let filterModalJson = this.customForms.getModelJsonKey(this.status.filterModal, false);
    let loadParams = {
      maxResultCount: 8
    };
    this.loadDatas({
  ...
  public toolsClickEvent(res: any) {
    let tool = res.tool; let data = res.data;
    if (tool.type == 'form') {
      // 编辑
