  ...
  public addSave() {
    ...
    if (activedNode) {
      ...
      this.status.refreshTreeNodeFn = (flag, result?) => {
        if(flag){
          let nodeData = {title: value, key: result ? result.result : '', isLeaf: true};
      ...
      this.status.refreshTreeNodeFn = (flag, result?) => {
        if (flag) {
          ...
          this.status.reloadTree = false;
          setTimeout(() => {
            this.status.treeNodes.push({title: value, key: result ? result.result : '', isLeaf: true, children: null});
            this.status.reloadTree = true;
          }, 0);
        }
        // 不管成功或失败，重新启用表单元素
